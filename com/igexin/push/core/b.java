package com.igexin.push.core;

/* loaded from: source-7994992-dex2jar.jar:com/igexin/push/core/b.class */
public final class b {
    public static final String A = "pushmessage";
    public static final String B = "received";
    public static final String C = "redirect_server";
    public static final long D = 60000;
    public static final long E = 3600000;
    public static final long F = 86400000;
    public static final String G = "com.igexin.sdk.action.";
    public static final String H = "com.igexin.sdk.action.doaction";
    public static final String I = "com.igexin.extension.message.action";
    public static final String J = "android.intent.action.TIME_SET";
    public static final String K = "17258000";
    public static final int L = 20000;
    public static int M = 0;
    public static int N = 3;
    public static int O = 5;
    public static int P = 7;
    public static int Q = 10;
    public static int R = 11;
    public static int S = 12;
    public static final String T = "6T5@S_";
    public static final String U = "config";
    public static final String V = "runtime";
    public static final String W = "message";
    public static final String X = "ral";
    public static final String Y = "ca";
    public static final String Z = "bi";

    /* renamed from: a  reason: collision with root package name */
    public static final String f23426a = "BUILD_CONFIG";
    public static final String aa = "st";
    public static final String ab = "bidata";
    public static int ac = 0;
    public static int ad = 1;
    public static int ae = 2;
    public static int af = 1;
    public static int ag = 2;
    public static int ah = 3;
    public static final String ai = "cantgetimei";
    public static final long aj = 8;
    public static final String ak = ",";
    public static String al = "com.igexin.sdk.PushService";
    public static String am = "com.igexin.sdk.PushService";
    public static final int an = 200;
    public static final int ao = 100;
    public static final int ap = 150;
    public static final int aq = 5;

    /* renamed from: ar  reason: collision with root package name */
    public static final int f23427ar = 1000;
    public static final int as = 250;
    public static final long at = 800;
    public static final String au = "postAction";
    public static final String av = "feedbackAction";
    public static final int aw = 0;
    public static final int ax = 1;
    public static final String ay = "pkg";
    public static final String b = "PUSH_APPID";

    /* renamed from: c  reason: collision with root package name */
    public static final String f23428c = "GETUI_APPID";
    public static final String d = "GT_INSTALL_CHANNEL";
    public static final String e = "EXT-";
    public static final String f = "0";
    public static final String g = "1";
    public static final String h = "40550";
    public static final String i = "GT";
    public static final String j = "CACHE_";
    public static final String k = "EXEC_";
    public static final String l = "null";
    public static final String m = "notification";
    public static final String n = "terminatetask";
    public static final String o = "startmyactivity";
    public static final String p = "startapp";
    public static final String q = "goto";
    public static final String r = "startweb";
    public static final String s = "checkapp";
    public static final String t = "cleanext";
    public static final String u = "enablelog";
    public static final String v = "disablelog";
    public static final String w = "targetpkgname";
    public static final String x = "ok";
    public static final String y = "id";
    public static final String z = "action";
}
