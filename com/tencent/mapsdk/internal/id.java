...
public class id extends GroundOverlayInfo implements vc {
    ...
    public void a() {
        if (this.f37554a.getPosition() == null) {
            return;
        }
        float max = Math.max(0.0f, Math.min(1.0f, this.f37554a.getAnchorU()));
        float max2 = Math.max(0.0f, Math.min(1.0f, this.f37554a.getAnchorV()));
        double pow = Math.pow(2.0d, 20.0f - Math.max(3.0f, Math.min(22.0f, this.f37554a.getZoom())));
        a6 a6Var = new a6(2.68435456E8d);
        p5 c2 = a6Var.c(this.f37554a.getPosition());
        this.mLatLngBounds = LatLngBounds.builder().include(a6Var.b(new p5(c2.b - ((this.mBitmapWidth * max) * pow), c2.f37683c - ((this.mBitmapHeight * max2) * pow)))).include(a6Var.b(new p5(c2.b + (this.mBitmapWidth * (1.0d - max) * pow), c2.f37683c + (this.mBitmapHeight * (1.0d - max2) * pow)))).build();
    }
