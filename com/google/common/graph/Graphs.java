...
/* loaded from: source-8110460-dex2jar.jar:com/google/common/graph/Graphs.class */
public final class Graphs {

    /* JADX INFO: Access modifiers changed from: package-private */
    /* loaded from: source-8110460-dex2jar.jar:com/google/common/graph/Graphs$NodeVisitState.class */
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    /* loaded from: source-8110460-dex2jar.jar:com/google/common/graph/Graphs$TransposedGraph.class */
    ...
    /* loaded from: source-8110460-dex2jar.jar:com/google/common/graph/Graphs$TransposedNetwork.class */
    ...
    /* loaded from: source-8110460-dex2jar.jar:com/google/common/graph/Graphs$TransposedValueGraph.class */
    ...
    public static <N> MutableGraph<N> inducedSubgraph(Graph<N> graph, Iterable<? extends N> iterable) {
        MutableGraph<N> build = iterable instanceof Collection ? GraphBuilder.from(graph).expectedNodeCount(((Collection) iterable).size()).build() : GraphBuilder.from(graph).build();
        for (N n : iterable) {
            build.addNode(n);
        }
        for (N n2 : build.nodes()) {
            for (N n3 : graph.successors((Graph<N>) n2)) {
                if (build.nodes().contains(n3)) {
                    build.putEdge(n2, n3);
                }
            }
        }
    ...
    public static <N, E> MutableNetwork<N, E> inducedSubgraph(Network<N, E> network, Iterable<? extends N> iterable) {
        MutableNetwork<N, E> build = iterable instanceof Collection ? NetworkBuilder.from(network).expectedNodeCount(((Collection) iterable).size()).build() : NetworkBuilder.from(network).build();
        for (N n : iterable) {
            build.addNode(n);
        }
        for (N n2 : build.nodes()) {
            for (E e : network.outEdges(n2)) {
                ...
                if (build.nodes().contains(adjacentNode)) {
                    build.addEdge(n2, adjacentNode, e);
                }
            }
        }
        return build;
    ...
    public static <N, V> MutableValueGraph<N, V> inducedSubgraph(ValueGraph<N, V> valueGraph, Iterable<? extends N> iterable) {
        MutableValueGraph<N, V> build = iterable instanceof Collection ? ValueGraphBuilder.from(valueGraph).expectedNodeCount(((Collection) iterable).size()).build() : ValueGraphBuilder.from(valueGraph).build();
        for (N n : iterable) {
            build.addNode(n);
        }
        for (N n2 : build.nodes()) {
            for (N n3 : valueGraph.successors((ValueGraph<N, V>) n2)) {
                if (build.nodes().contains(n3)) {
                    build.putEdgeValue(n2, n3, valueGraph.edgeValueOrDefault(n2, n3, null));
                }
            }
        }
        return build;
            ...
            for (N n2 : graph.nodes()) {
                if (!hashSet.contains(n2)) {
                    ...
                    while (true) {
                        ...
                        if (it.hasNext()) {
                            Object next = it.next();
                            for (Object obj2 : Iterables.limit(reachableNodes, i2)) {
                                build.putEdge(next, obj2);
                            }
                            i = i2 + 1;
                        }
                    }
